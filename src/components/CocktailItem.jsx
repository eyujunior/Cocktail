  ...
  return (
    <div className='relative'>
      <svg
        ...
        className={`icon icon-add w-12 h-12 absolute top-2 right-2 bg-green-600 fill-white stroke-white p-2 ${
      ...
      <Link to={`/cocktail/${cocktailItem.idDrink}`}>
        <div className='bg-white text-black text-center rounded-md overflow-hidden'>
          <img
          ...
          <div className='flex items-center justify-between p-4 text-xl'>
            <p className='text-2xl font-bold'>{cocktailItem.strDrink}</p>
            <p>{cocktailItem.strCategory}</p>
          </div>
        </div>
      </Link>
    </div>
